...
import { Link as LinkIcon, Copy, ArrowRight, Sparkles, Check, ChevronLeft } from "lucide-react";
...
const PlayPrivate = () => {
  ...
  const shareUrl = `${window.location.origin}/game/private?room=${roomCode}&color=${color}&time=${timeControl}`;
  ...
  const handleJoin = () => {
    ...
    navigate(`/game/private?room=${roomCode}&color=${finalColor}&time=${timeControl}`);
  };
            ...
            <div className="relative z-10">
              ...
              <div className="mb-6 sm:mb-8">
                ...
                <p className="text-xs sm:text-sm text-foreground-muted mt-1.5 font-medium">
                  Generate a challenge code, invite a friend, and play on your own rules.
                </p>
              </div>
                  ...
                  {(["white", "random", "black"] as ColorChoice[]).map((c) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[10px] sm:text-xs font-semibold">{c}</span>
                      </button>
                    );
                  })}
                </div>
                  ...
                  {TIME_CONTROLS.map((tc) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[9px] sm:text-[10px] text-foreground-muted mt-0.5 font-medium truncate">{tc.desc}</span>
                      </button>
                    );
                  })}
                </div>
              ...
              {!roomCode ? (
                ...
                >
                  <Sparkles className="w-5 h-5 animate-pulse" /> Generate Challenge Link
                </button>
              ) : (
                <div className="space-y-4 bg-slate-900/50 border border-border p-4 rounded-xl animate-slide-up">
                  <div>
                    <span className="text-xs text-foreground-muted font-bold tracking-wider uppercase">Challenge Link Generated</span>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2.5 border-t border-border/45">
                    <div className="flex items-center justify-between sm:justify-start w-full sm:w-auto">
                      <span className="text-[10px] text-foreground-muted font-bold uppercase tracking-wider">Room Code:</span>
                      <span className="text-sm font-bold text-primary font-mono ml-1.5 tracking-wider">{roomCode}</span>
                    </div>
                    <button
                      onClick={handleJoin}
